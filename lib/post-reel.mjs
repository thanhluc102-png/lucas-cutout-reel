...
// Đăng một video render sẵn với caption/comment tự soạn: đặt CAPTION_FILE +
// COMMENT_FILE thì lấy nội dung từ đó, khỏi cần build/data.json (pipeline lib/).
// Không đặt thì giữ nguyên hành vi cũ — dựng caption từ dữ liệu sản phẩm.
const captionFile = process.env.CAPTION_FILE;
const commentFile = process.env.COMMENT_FILE;
let caption, commentText, productName = '';
if (captionFile && commentFile && (await stat(captionFile).catch(() => false))) {
  caption = (await readFile(captionFile, 'utf8')).trimEnd();
  commentText = (await readFile(commentFile, 'utf8')).trimEnd();
} else if (await stat('build/data.json').catch(() => false)) {
  const data = JSON.parse(await readFile('build/data.json', 'utf8'));
  caption = buildCaption(data);
  commentText = buildComment(data);
  productName = data.name ?? '';
} else {
  caption = 'Sản phẩm phụ kiện chính hãng tại lucas.vn 🔥 #LucasCombo #phukienapple #chinhhang';
  commentText = 'Đặt hàng tại: https://lucas.vn';
}
...
try {
  ...
  const body = new URLSearchParams({ message: commentText });
...
// Ghi lịch sử để vòng tự học (sync-insights + optimize) đo tương tác về sau.
try {
  let hist = [];
  try { hist = JSON.parse(await readFile('reels_history.json', 'utf8')); } catch { /* lần đầu */ }
  ...
  await writeFile('reels_history.json', JSON.stringify(hist, null, 2), 'utf8');
  console.log('ghi   reels_history.json');
} catch (err) {
  console.warn(`[warn] không ghi được reels_history.json: ${err.message}`);
}
